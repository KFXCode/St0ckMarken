...
function scenarioHTML(p,q){
 ...
 if(p.kind==='CASH-SECURED PUT'){
  return '<div class="scn"><span>Stays above $'+p.strike+' strike</span><span style="color:#34d399">keep +'+fmt(p.prem*100*q)+' premium</span></div>'+
         '<div class="scn"><span>Closes below $'+p.strike+'</span><span style="color:#fb7185">assigned '+ (100*q)+' shares at $'+p.strike+'</span></div>'+
 ...
 [1,3,5].forEach(function(m){
  ...
  rows+='<div class="scn"><span>'+fav+' '+m+'%</span><span style="color:#34d399">+'+fmt(win)+'</span></div>'+
        '<div class="scn"><span>'+adv+' '+m+'%</span><span style="color:#fb7185">−'+fmt(loss)+'</span></div>';
 });
 rows+='<div class="scn"><span>Worst case (expires worthless)</span><span style="color:#fb7185">−'+fmt(cost)+'</span></div>'+
...
function render(){var st=state();
 ...
 ge.textContent=(gr>=0?'+':'')+gr.toFixed(1)+'%';ge.style.color=gr>=0?'#34d399':'#fb7185';
 txt('sm-cash','Cash available: '+fmt(st.cash)+(openVal?' · tied up in open trades: '+fmt(openVal):''));
 if(hpv){hpv.textContent=fmt(now);hpv.style.fontSize='38px';}
 renderGoal(now);
 renderWeekly(st);
 if(hpc){hpc.textContent=(gr>=0?'▲ +':'▼ ')+fmt(now-st.start)+' ('+(gr>=0?'+':'')+gr.toFixed(1)+'%) all time';hpc.style.color=gr>=0?'#00c805':'#ff5000';}
 ...
 document.getElementById('sm-closed').innerHTML=st.closed.length?
  '<div class="why">Closed trades</div>'+st.closed.map(function(p){
   return '<div class="pos">'+p.t+' '+p.kind+' — '+p.result+' <span style="color:'+(p.pl>=0?'#34d399':'#fb7185')+'">'+(p.pl>=0?'+':'')+p.pl+'%</span> → back '+fmt(p.proceeds)+'</div>'}).join(''):'';
 markTaken(st)}
...
function wire(){document.querySelectorAll('.take').forEach(function(el){
 ...
 var p={t:d.t,cost:+d.cost,prem:+d.prem,spot:+d.spot,kind:d.kind,date:d.date,
        strike:d.strike,exp:d.exp,action:d.action||'BUY'};
 var qEl=el.querySelector('.tq'),pot=el.querySelector('.pot'),q=1;
 function upd(){qEl.textContent=q;pot.innerHTML=scenarioHTML(p,q)}
 upd();
 ...
 if(tk)tk.onclick=function(){var st=state();
  if(st.start==null){alert('Set your starting budget first — see the My Budget section below.');return}
  var cost=p.cost*q;
 ...
 if(bb)bb.onclick=function(){
  var bk=g('broker');
  if(!bk||!BROKERS[bk]){alert('Pick your broker first — there\'s a "Your broker" dropdown in the My Budget section below. Then this button copies the order and opens the ticker at your broker.');return}
  var order=orderText(p,q);
  var go=function(){window.open(BROKERS[bk].url(p.t),'_blank');
...
function renderWeekly(st){
 ...
 order.forEach(function(k){var wk=weeks[k],startB=bal,endB=bal+wk.net,pct=startB?wk.net/startB*100:0;bal=endB;
  var col=wk.net>=0?'#00c805':'#ff5000';
 ...
 box.innerHTML='<div class="why">📈 Weekly progress</div>'+rows+
  '<div class="hint" style="margin-top:6px">Since day one: <b style="color:'+(tot>=0?'#00c805':'#ff5000')+'">'+(tot>=0?'+':'')+fmt(tot)+' ('+(totp>=0?'+':'')+totp.toFixed(0)+'%)</b> — this is your money compounding.</div>';
}
function renderGoal(now){
 ...
 var msg,color;
 if(need<=10){msg='Ambitious but doable over time. Steady singles get you here.';color='#00c805'}
 else if(need<=30){msg='Very aggressive — expect losing weeks. Only risk money you can lose.';color='#f5b84c'}
 else{msg='Not realistic. That would mean roughly +'+Math.round(need)+'% EVERY week, which no system can do reliably. Lower the goal or grow the bankroll first.';color='#ff5000'}
...
if(sb)sb.onclick=function(){var v=parseFloat(document.getElementById('sm-amt').value);
 if(!(v>0)){alert('Enter a starting amount, e.g. 100.');return}
 s('start',v);s('cash',v);s('open',[]);s('closed',[]);render()};
var rb=document.getElementById('sm-reset');
if(rb)rb.onclick=function(){if(confirm('Reset your budget tracker? This clears your simulated bankroll and trade history on this device.')){
